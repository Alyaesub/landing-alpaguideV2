...
function trapFocus(event) {
  ...
  const focusables = navFocusable.length ? navFocusable : Array.from(nav.querySelectorAll(focusableSelector));
...
function handleForm(form) {
  ...
  form.addEventListener('submit', (event) => {
    ...
    const email = (formData.get('email') || formData.get('cta-email') || '').toString().trim();
